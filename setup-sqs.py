...
def write_config_toml(config_data: Dict[str, Any], config_path: str = 'config.toml') -> bool:
    """
    ...
    """
    try:
        # Build TOML content manually (avoiding external dependency)
        toml_content = "# SQS Integration Configuration\n"
        toml_content += "# Generated by setup-sqs.py\n\n"

        toml_content += "[sqs]\n"
        toml_content += f"queue_url = \"{config_data['queue_url']}\"\n"
        toml_content += f"queue_arn = \"{config_data['queue_arn']}\"\n"
        toml_content += f"region = \"{config_data['region']}\"\n\n"

        toml_content += "[stack]\n"
        toml_content += f"name = \"{config_data['stack_name']}\"\n"
        toml_content += f"catalog_url = \"{config_data['catalog_url']}\"\n"
        if config_data.get('account_id'):
            toml_content += f"account_id = \"{config_data['account_id']}\"\n"
        toml_content += "\n"

        toml_content += "[roles]\n"
        toml_content += f"updated = {json.dumps(config_data.get('updated_roles', []))}\n"
...
def main():
    ...
    print("4. Run ./check-sqs.sh to verify the configuration")
    print()
